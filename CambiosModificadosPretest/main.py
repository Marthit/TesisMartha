...
def pre_diagnostico2():
     
     cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
     check_exists_sql = "SELECT pregunta as question,valor,imagen FROM preguntas  WHERE id_examen= 1"
     cursor.execute(check_exists_sql)
     preguntas = cursor.fetchall()
     print(preguntas)
     check_exists_sql = "SELECT*from pregunta_detalle "
     cursor.execute(check_exists_sql)
     opciones = cursor.fetchall()
     return render_template('quiz.html',  preguntas=preguntas,opciones=opciones)
    # El usuario no ha iniciado sesión, redirigir a la página de inicio de sesión
     return redirect(url_for('login'))   

@app.route('/pre_diagnostico', methods=['GET', 'POST'])
def pre_diagnostico():
     if  request.method == 'GET'  in request.form:
         cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
         check_exists_sql = "SELECT * FROM preguntas WHERE id_examen= 1"
         cursor.execute(check_exists_sql)
         preguntas = cursor.fetchall()
         print(preguntas)
     else:
        if 'loggedin' in session:    
            ...
            respuestas = [1,0,0,1,0,0,2,0,2,1,1,0,1,0,0,1,0,1,0,2,1] #Respuestas correctas
            selections = request.args.get('selections')
            print(selections)
            print('Martha Angélica')
            ...
            print ("res:",res)
            for elem in res: 
                print(respuestas[i] ," - ", elem, "Martha") #(respuesta correctas - respuestas del estudiante)
            ...
            if(numCorrect<12):
                msg= 'El estudiante inicia con un nivel C en Inglés:En Inicio. <br><br> Se recomienda que revise el tema "Los Adjetivos y tiempos Verbales" para reforzar su aprendizaje en Ingles'
            elif(numCorrect<14):
                msg= 'El estudiante inicia con un nivel B en Inglés: En Desarrollo.<br><br> Se recomienda que revise el tema "Los Adjetivos '
            elif(numCorrect<18):
                msg= 'El estudiante inicia con un nivel A en Inglés: En Previsto.<br><br> Se recomienda  "Sigue Practicando!!!!'        
            else:
                msg= 'El estudiante inicia con un nivel AD en Inglés:Destacado.<br><br>  "Biennn!!!! '        
                                    
            msg+= ' -->Y posee un puntaje en el test de: '+ str(numCorrect)                                             
            msg+= "<br><br> Se le recomienda ver los siguientes videos de Abjetivos:"
            msg+= "<br><br>&nbsp; &nbsp; &nbsp; &nbsp;<iframe width='460' height='315' src='https://www.youtube.com/embed/eHsIKoNiTwI' frameborder='0' allow='accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture' allowfullscreen></iframe>"
            msg+= "&nbsp; &nbsp; &nbsp; &nbsp;&nbsp; &nbsp; &nbsp; <iframe width='450' height='315' src='https://www.youtube.com/embed/CqujZ4YEH80' frameborder='0' allow='accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture' allowfullscreen></iframe>"
            ...
            #Guardar pre diagnóstico
            
            #cursor.execute('INSERT INTO resultados(`id_cuenta`,`id_pregunta`,`respuesta_pre_test`) VALUES(1,1,1);')
    
            #hacer tipo for para guardar todas la preguntas del pre test
            id_pregunta=1
            for elem in res: 
                print('usuarioId: ',session['id'] ,"idPregunta: ",id_pregunta, "Respuesta alumno: ", elem) #(respuesta correctas - respuestas del estudiante)
                cursor.execute('INSERT INTO resultados(`id_cuenta`,`id_pregunta`,`respuesta_pre_test`) VALUES(%s,%s,%s);', (session['id'],id_pregunta,elem))
                id_pregunta=id_pregunta+1 
            
            
            #Fin guardar pre diagnóstico
...
def post_diagnostico():
    ...
    if 'loggedin' in session:    
        ...
        if(nivel_competencia>18):
            ...
            msg+= "<br><br> Se le recomienda  seguir practicando!!:" 
